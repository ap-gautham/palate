import type { Catalog, CriticMatch, Movie } from "./types";

const DECILES = 10;

// Mirrors rotten_tomatoes/movie_features.py's GENRE_VOCAB_K (19 canonical
// genres + "__other__" = 20 slots).
...
function actorBlock(
  ...
  for (let i = 0; i < 5; i++) {
    ...
    if (i < byRating.length) {
      row[zCol] = NaN; // filled below once muU/sigmaU known
...
/** Mirrors features.py:decile_features -- summarize
...
// a critic across a decile boundary and perturbs XGBoost/NN output vs the
// Python app -- confirmed negligible against the offline Python predict path
// in web/scripts/validate.ts.
 ...
 * features.py:app_features (single-movie body). */
